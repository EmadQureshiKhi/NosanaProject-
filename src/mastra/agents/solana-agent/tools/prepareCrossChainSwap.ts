...
async function resolveTokenContract(tokenSymbol: string, chain: string): Promise<string | null> {
  ...
  // For Solana, use Jupiter search
  if (chain.toLowerCase() === "solana") {
    try {
      const results = await searchJupiterTokens(tokenSymbol);
      if (results.length > 0) {
        console.log(`✅ Found Solana token via Jupiter: ${results[0].address}`);
...
export const prepareCrossChainSwap = createTool({
  ...
  inputSchema: z.object({
    ...
    slippageBps: z.union([z.number(), z.string()]).optional().default("auto").describe("Slippage in basis points or 'auto'"),
  ...
  execute: async (args) => {
    ...
    const slippageBps = args.slippageBps || args.input?.slippageBps || "auto";
